...
class Logger {
    ...
    constructor(config) {
        ...
        const fileDebug = new FileLogger({groupName: 'DEBUG:', file: `${config.rootPath}/${config.debugFile}`}, levels.indexOf('debug'), level);
        const fileInfo = new FileLogger({groupName: 'INFO:', file: `${config.rootPath}/${config.infoFile}`}, levels.indexOf('info'), level);
        const fileWarn = new FileLogger({groupName: 'WARN:', file: `${config.rootPath}/${config.warnFile}`}, levels.indexOf('warn'), level);
        const fileError = new FileLogger({groupName: 'ERROR:', file: `${config.rootPath}/${config.errorFile}`}, levels.indexOf('error'), level);
